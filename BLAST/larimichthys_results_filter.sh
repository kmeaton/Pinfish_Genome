...
blastfile="./MAKER_proteins_croaker.OUT" # the name of your output table from BLAST
goinfo="croaker_gene_GO.tsv" # the name of a file containing gene IDs and descriptions from the species you BLASTED against
...
cat $blastfile | awk -v mincov="$mincoverage" '$13 > mincov {print $0}' | awk -v minid="$minpercentid" '$3 >= minid {print $0}' | awk -v ev="$evalue" '$11 < ev {print $0}' > croaker_multiple_matches.txt
...
cat croaker_multiple_matches.txt | awk '{print $1}' | sort | uniq > croaker_annotated_transcripts.txt
...
for transcript in `cat croaker_annotated_transcripts.txt`
do
    egrep "$transcript\s" croaker_multiple_matches.txt > temp.txt
    sort -nr -k 12 temp.txt > sorted.txt
    head -1 sorted.txt >> croaker_one_annotation_per_transcript.txt
    rm temp.txt
    rm sorted.txt
done
...
cat croaker_one_annotation_per_transcript.txt | awk '{print $1,$2,$3,$11,$12,$13}' | sed -r 's/\s+/,/g' > croaker_ids.txt
...
for gene in `cat croaker_ids.txt`
do
        ...
        grep "$EI" $goinfo | sed "s/^ENS/$TRINITY       $PID    $EVAL   $BIT    $COV    ENS/g" >> croaker_annotation_table.tsv
done
...
seqkit grep -v -f croaker_annotated_transcripts.txt sparus_unannotated_proteins.fa -o croaker_unannotated_proteins.fa
