...
blastfile="./MAKER_proteins_zebrafish.OUT" # the name of your output table from BLAST
goinfo="zebrafish_gene_GO.tsv" # the name of a file containing gene IDs and descriptions from the species you BLASTED against
...
cat $blastfile | awk -v mincov="$mincoverage" '$13 > mincov {print $0}' | awk -v minid="$minpercentid" '$3 >= minid {print $0}' | awk -v ev="$evalue" '$11 < ev {print $0}' > zebrafish_multiple_matches.txt
...
cat zebrafish_multiple_matches.txt | awk '{print $1}' | sort | uniq > zebrafish_annotated_transcripts.txt
...
for transcript in `cat zebrafish_annotated_transcripts.txt`
do
    egrep "$transcript\s" zebrafish_multiple_matches.txt > temp.txt
    sort -nr -k 12 temp.txt > sorted.txt
    head -1 sorted.txt >> zebrafish_one_annotation_per_transcript.txt
    rm temp.txt
    rm sorted.txt
done
...
cat zebrafish_one_annotation_per_transcript.txt | awk '{print $1,$2,$3,$11,$12,$13}' | sed -r 's/\s+/,/g' > zebrafish_ids.txt
...
for gene in `cat zebrafish_ids.txt`
do
        ...
        grep "$EI" $goinfo | sed "s/^ENS/$TRINITY       $PID    $EVAL   $BIT    $COV    ENS/g" >> zebrafish_annotation_table.tsv
done
...
seqkit grep -v -f zebrafish_annotated_transcripts.txt croaker_unannotated_proteins.fa -o final_unannotated_proteins.fa
